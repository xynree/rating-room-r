<script lang="ts">
  ...
  async function createItem(e: { detail: EditState }) {
    ...
    await invoke("add_categories_to_item", { itemId: newItemId, categories: editState.categories })
    await invoke("create_rating", { rating: editState.rating, itemId: newItemId })
